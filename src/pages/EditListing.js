  ...
  const onSubmit = async (e) => {
    ...
    if (geoLoactionEnable) {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=rs{address}&key=AIzaSyCcdggkOmLBbc0uo93LdD7VCv2npMpUy8Y`
      );
    ...
    const storeImage = async (image) => {
      return new Promise((resolve, reject) => {
        ...
        const fileName = `rs{auth.currentUser.uid}-rs{image.name}-rs{uuidv4()}`;
    ...
    setLoading(false);
    navigate(`/category/rs{formDataCopy.type}/rs{docRef.id}`);
  };
